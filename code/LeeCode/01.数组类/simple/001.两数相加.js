...
const twoSum = (nums, target) => {
  ...
  let second = 0
  let loopNum = 0
  let oldArr = JSON.parse(JSON.stringify(nums))
  let len = oldArr.length
  for (let i = 0; i < len; i++) {
    loopNum++
    first = i
    tempNum = target - oldArr[i]
    console.log('外部的剩余数组内容', i, oldArr[i], oldArr, '对比值：', tempNum,'第几轮循环',loopNum)
    nums.splice(0, 1)
    console.log('新数组：', nums)
    
    for (let j = 0; j < nums.length; j++) {
      console.log('====数组内容', j, nums[j], nums)
      if (tempNum == nums[j]) {
        console.log('成功通关', j)
        second = j + loopNum
        targetArr.push(first, second)
        return [first, second]
      } else {
        console.log('对比失败', j, nums[j])
        
      }
    }
    console.log('结束一个周期************', i, tempNum, nums,'第几轮循环',loopNum)
    // let mySetObj = new Set([...nums, tempNum]);
    // console.log('mySetObj', mySetObj)
    // let mySetArr = [...mySetObj]
    // let tempLen = mySetArr.length
    // // console.log('mySetArr', mySetArr)
    // console.log(len, '==', tempLen)

  }
  // console.log(nums)
  // second = nums.findIndex(item => item == tempNum)
  // targetArr.push(first, second)
  // return [first, second]
};
console.time("twoSum");
